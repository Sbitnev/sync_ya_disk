...
### Ограничения загрузки файлов (config.py)

Эти параметры контролируют, какие файлы загружаются с Яндекс.Диска:

| Переменная | Тип | Значение по умолчанию | Описание |
|------------|-----|----------------------|----------|
| **SKIP_LARGE_FILES** | bool | True | Пропускать файлы больше MAX_FILE_SIZE |
| **MAX_FILE_SIZE** | int | 1073741824 (1 ГБ) | Максимальный размер файла для загрузки |
| **MAX_TABULAR_FILE_SIZE** | int | 3145728 (3 МБ) | Максимальный размер табличных файлов (.csv, .xlsx, .xls, .xlsm, .xlsb, .parquet). Используется минимум из MAX_FILE_SIZE и MAX_TABULAR_FILE_SIZE |
| **ENABLE_TOTAL_SIZE_LIMIT** | bool | True | Применять ограничение на общий размер загрузки |
| **MAX_TOTAL_SIZE** | int | 107374182400 (100 ГБ) | Максимальный общий размер скачиваемых файлов |
| **SKIP_VIDEO_FILES** | bool | True | Пропускать видео файлы (не загружать) |
| **VIDEO_EXTENSIONS** | list | ['.mp4', '.avi', '.mov', ...] | Расширения видео файлов |
| **SKIP_IMAGE_FILES** | bool | True | Пропускать изображения (не загружать) |
| **IMAGE_EXTENSIONS** | list | ['.png', '.jpg', '.jpeg', ...] | Расширения файлов изображений |
| **SKIP_PARQUET_FILES** | bool | True | Пропускать Parquet файлы (экономия трафика и места) |
| **PARQUET_EXTENSIONS** | list | ['.parquet'] | Расширения Parquet файлов |
| **SKIP_FILE_PATTERNS** | list | [r'^~\$', r'\.tmp$', ...] | Regex паттерны для пропуска файлов (временные, .drawio, .mpp, .epf, .mht, .mp3) |

### Ограничения конвертации файлов (config.py)

Эти параметры контролируют, как файлы конвертируются в Markdown:

| Переменная | Тип | Значение по умолчанию | Описание |
|------------|-----|----------------------|----------|
| **ENABLE_MARKDOWN_CONVERSION** | bool | True | Главный переключатель конвертации (отключает всё) |
| **DELETE_ORIGINALS_AFTER_CONVERSION** | bool | True | Удалять оригинальные файлы после конвертации |
| **MARKDOWN_OUTPUT_DIR** | Path | localdata/markdown_files | Папка для сохранения .md файлов |

#### Word документы
| Переменная | Тип | Значение по умолчанию | Описание |
|------------|-----|----------------------|----------|
| **CONVERT_WORD_FILES** | bool | True | Конвертировать .docx и .doc файлы |
| **REPLACE_IMAGES_WITH_PLACEHOLDERS** | bool | True | Заменять изображения текстовыми заглушками |

#### CSV файлы
| Переменная | Тип | Значение по умолчанию | Описание |
|------------|-----|----------------------|----------|
| **CONVERT_CSV_FILES** | bool | True | Конвертировать .csv файлы |
| **CSV_MAX_ROWS** | int/None | None | **⚠️ КРИТИЧНО:** Ограничение строк (None = загружает весь файл в память!) |
| **CSV_MAX_COLUMNS** | int/None | None | Ограничение столбцов |

#### Excel файлы
| Переменная | Тип | Значение по умолчанию | Описание |
|------------|-----|----------------------|----------|
| **CONVERT_EXCEL_FILES** | bool | True | Конвертировать .xlsx, .xls, .xlsm, .xlsb файлы |
| **EXCEL_TO_CSV** | bool | True | Сохранять как CSV вместо Markdown |
| **EXCEL_MAX_ROWS** | int/None | None | **⚠️ КРИТИЧНО:** Ограничение строк (None = загружает весь файл в память!) |
| **EXCEL_MAX_COLUMNS** | int/None | None | Ограничение столбцов |
| **EXCEL_MAX_SHEETS** | int/None | None | Ограничение количества листов |

#### PDF документы
| Переменная | Тип | Значение по умолчанию | Описание |
|------------|-----|----------------------|----------|
| **CONVERT_PDF_FILES** | bool | True | Конвертировать .pdf файлы |
| **PDF_MAX_PAGES** | int | 100 | Максимальное количество страниц для конвертации |

#### PowerPoint презентации
| Переменная | Тип | Значение по умолчанию | Описание |
|------------|-----|----------------------|----------|
| **CONVERT_POWERPOINT_FILES** | bool | True | Конвертировать .pptx и .ppt файлы |

#### Прочие форматы
| Переменная | Тип | Значение по умолчанию | Описание |
|------------|-----|----------------------|----------|
| **CONVERT_TEXT_FILES** | bool | True | Конвертировать .txt, .md, .py, .json, .xml, .avsc, .j2 |
| **CONVERT_HTML_FILES** | bool | True | Конвертировать .html и .htm файлы |
| **CONVERT_RTF_FILES** | bool | True | Конвертировать .rtf файлы |

#### Архивы
| Переменная | Тип | Значение по умолчанию | Описание |
|------------|-----|----------------------|----------|
| **CONVERT_ARCHIVE_FILES** | bool | True | Конвертировать .zip, .7z, .rar, .tar, .gz, .tgz (распаковка + конвертация содержимого) |
| **ARCHIVE_MAX_DEPTH** | int | 10 | Максимальная глубина вложенности архивов |

#### Parquet файлы
| Переменная | Тип | Значение по умолчанию | Описание |
|------------|-----|----------------------|----------|
| **CONVERT_PARQUET_FILES** | bool | False | Конвертировать .parquet файлы (по умолчанию отключено) |
| **PARQUET_TO_CSV** | bool | True | Сохранять как CSV вместо Markdown |
| **PARQUET_MAX_ROWS** | int/None | None | **⚠️ КРИТИЧНО:** Ограничение строк (None = загружает весь файл в память!) |
| **PARQUET_MAX_COLUMNS** | int/None | None | Ограничение столбцов |

### Рекомендации по настройке для серверов с ограниченной памятью

**⚠️ КРИТИЧНО для серверов с 8GB RAM или меньше:**

```python
# === ОБЯЗАТЕЛЬНО установите лимиты строк ===
CSV_MAX_ROWS = 10000        # вместо None - предотвращает загрузку гигантских CSV в память
EXCEL_MAX_ROWS = 10000      # вместо None - предотвращает загрузку гигантских Excel в память
PARQUET_MAX_ROWS = 10000    # вместо None - предотвращает загрузку гигантских Parquet в память

# === Уменьшите количество потоков ===
MAX_WORKERS = 1             # вместо 2 - меньше одновременных операций
FOLDER_SCAN_WORKERS = 3     # вместо 10 - меньше параллельного сканирования

# === Отключите архивы (занимают много памяти) ===
CONVERT_ARCHIVE_FILES = False

# === Уменьшите размеры пулов ===
HTTP_POOL_CONNECTIONS = 10  # вместо 30
HTTP_POOL_MAXSIZE = 20      # вместо 50
```

**Пример использования:**

```python
# Ограничить только табличные файлы, остальные до 1 ГБ
MAX_FILE_SIZE = 1 * 1024 * 1024 * 1024  # 1 ГБ для всех файлов
MAX_TABULAR_FILE_SIZE = 5 * 1024 * 1024  # 5 МБ для CSV/Excel/Parquet
# Табличный файл будет пропущен если > 5 МБ (минимум из двух значений)
# Word/PDF файл будет пропущен если > 1 ГБ
```
