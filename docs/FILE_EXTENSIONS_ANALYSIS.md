...
| .avsc | ⚙️ Планируется | Avro Schema (JSON) - форматировать как code block |
| .bkp | ❌ Нет | Резервная копия - обычно не требуется |
| .csv | ✅ Да | **Pandas** → markdown таблицы (лимит: 1000 строк × 50 столбцов) |
| .doc | ✅ Да | **Pandoc** → markdown (старый формат Word) |
...
| .htm | ⚙️ Планируется | **html2text** или **pandoc** → markdown |
| .html | ⚙️ Планируется | **html2text** или **pandoc** → markdown |
| .j2 | ⚙️ Планируется | Jinja2 шаблон - форматировать как code block |
| .jpeg | ❌ Нет | Изображение - игнорируется (по требованию) |
| .jpg | ❌ Нет | Изображение - игнорируется (по требованию) |
| .json | ⚙️ Планируется | JSON данные - форматировать как code block |
| .lnk | ❌ Нет | Ярлык Windows - системный файл |
| .m4a | ❌ Нет | Аудио файл - только метаданные |
| .md | ✅ Да | Уже Markdown - копировать как есть |
...
| .parquet | ⚙️ Планируется | **Pandas** → CSV → markdown таблицы |
| .pbix | ❌ Нет | Power BI - проприетарный формат |
| .pdf | ⚙️ Планируется | **pdfplumber** или **PyPDF2** → markdown (текст без изображений) |
| .png | ❌ Нет | Изображение - игнорируется (по требованию) |
| .ppt | ⚙️ Планируется | **LibreOffice CLI** → markdown (каждый слайд = секция) |
| .pptx | ⚙️ Планируется | **python-pptx** → markdown (каждый слайд = секция) |
| .py | ⚙️ Планируется | Python скрипт - форматировать как code block |
| .rdp | ❌ Нет | Remote Desktop Protocol - конфигурационный файл |
| .rtf | ⚙️ Планируется | **Pandoc** → markdown |
...
| .txt | ⚙️ Планируется | Обернуть в markdown с метаданными |
| .webm | ❌ Нет | Видео файл - только метаданные |
| .xls | ⚙️ Планируется | **Pandas** → markdown (каждый лист = секция) |
| .xlsb | ⚙️ Планируется | **Pandas** → markdown (бинарный формат Excel) |
| .xlsm | ⚙️ Планируется | **Pandas** → markdown (макросы теряются) |
| .xlsx | ⚙️ Планируется | **Pandas** → markdown (каждый лист = секция) |
| .xml | ⚙️ Планируется | XML данные - форматировать как code block |
...
- ⚙️ **Планируется** - Возможна реализация в будущем
- ❌ **Нет** - Не преобразуется (технические ограничения или нецелесообразно)

### Статистика преобразования

**Текущее состояние (реализовано):**
- ✅ Word документы: 1,190 файлов (16.6%)
- ✅ CSV файлы: 507 файлов (7.1%)
- **Итого преобразуется**: 1,697 файлов (23.6%)

**Потенциал (при полной реализации):**
- ⚙️ Excel файлы: 3,512 файлов (48.9%)
- ⚙️ PDF документы: 468 файлов (6.5%)
- ⚙️ PowerPoint: 78 файлов (1.1%)
- ⚙️ Текстовые/код: ~80 файлов (1.1%)
- **Потенциальный итог**: 5,835 файлов (81.2%)
