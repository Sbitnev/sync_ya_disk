...
SKIP_FILE_PATTERNS = [
    ...
    r'\.m4a$',          # Аудио файлы M4A (не требуют конвертации)
    r'\.ogg$',          # Аудио файлы OGG (не требуют конвертации)
    r'\.pbix$',         # Power BI файлы (не поддерживаются)
    r'\.lnk$',          # Ярлыки Windows (не содержат данных)
    r'\.rdp$',          # Файлы удаленного рабочего стола (не содержат данных)
    r'\.sel$',          # Файлы запросов (специфический формат)
    r'\.erf$',          # Внешние отчеты 1С (не поддерживаются)
    r'\.snappy$',       # Snappy-сжатые файлы (требуют специальной обработки)
    r'^_SUCCESS$',      # Hadoop/Spark служебные файлы (маркеры успеха)
    r'^[0-9A-F]{8}$',   # Файлы без расширения с hex-именами (8 символов)
