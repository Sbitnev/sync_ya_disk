...
#### Поддержка старых .doc файлов
- Pandoc не поддерживает формат .doc (только .docx)
- Добавлена поддержка LibreOffice для конвертации .doc → .docx → markdown
- Если LibreOffice не установлен - .doc файлы пропускаются с информативным предупреждением
