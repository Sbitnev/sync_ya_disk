...
Для скачивания файлов из указанной папки без фильтрации, конвертации и использования БД:
...
- Использует настройки из `src/config.py`:
  - `REMOTE_FOLDER_PATH` - какую папку выгружать (по умолчанию "/Клиенты")
  - `USER_EMAIL` и `USER_ID` - от какого пользователя
- Скачивает все файлы из указанной папки в `stash/`
...
- **Оптимизированная производительность**:
  - 15 параллельных потоков для загрузки файлов
  - 20 параллельных потоков для сканирования структуры
  - Увеличенный HTTP connection pool (30/50)
  - Размер chunk 1 МБ для быстрой загрузки больших файлов
  - Специальный таймаут 15 сек для операций сканирования

**Пример вывода при запуске:**
```
======================================================================
ВЫГРУЗКА ФАЙЛОВ С ЯНДЕКС.ДИСКА В STASH/
======================================================================
Пользователь: tn@imprice.ai (ID: 1130000057842996)
Папка на диске: /Клиенты
Директория для скачивания: C:\path\to\sync_ya_disk\stash
Параллельных потоков: 15
======================================================================
```

**Примечание:** Папка `stash/` и файл прогресса добавлены в `.gitignore` и не будут коммититься в репозиторий.

#### Обработка файлов из stash в Markdown (без API запросов)

После выгрузки файлов в `stash/` можно обработать их локально без обращения к API:

```bash
python run_process_stash.py
```

Этот скрипт:
- Работает с уже скачанными файлами из `stash/`
- **НЕ делает запросы к API** Яндекс.Диска
- Конвертирует файлы в Markdown (в папку `stash_markdown/`)
- Применяет фильтры из `src/config.py`:
  - `SKIP_VIDEO_FILES` - пропускать видео
...
- Идеально для офлайн обработки больших архивов

**Сценарий использования:**
1. `python run_stash_download.py` - выгрузить все файлы без фильтров
2. `python run_process_stash.py` - обработать файлы локально с фильтрами
3. Результат: исходные файлы в `stash/`, Markdown в `stash_markdown/`

**Преимущества:**
- Разделение загрузки и обработки
- Не нужно держать подключение к API во время конвертации
- Можно перезапускать обработку без повторной загрузки
- Удобно для больших объемов данных

**Примечание:** Папка `stash_markdown/` добавлена в `.gitignore`.
...
├── run.py                      # Основной скрипт запуска (синхронизация с API)
├── run_stash_download.py       # Скрипт запуска полной выгрузки
├── run_process_stash.py        # Скрипт запуска обработки stash
├── download_all_to_stash.py    # Полная выгрузка всех файлов в stash/
├── process_stash.py            # Обработка файлов из stash в Markdown (без API)
├── convert_to_markdown.py      # Конвертация локальных файлов (для localdata)
├── requirements.txt            # Зависимости
├── .env                        # Переменные окружения (не в git)
├── stash/                      # Полная выгрузка диска (не в git)
├── stash_markdown/             # Обработанные файлы из stash (не в git)
