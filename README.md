...
| **CPU** | 2 ядра (2.0 GHz+) | Для FFmpeg и обработки документов |
| **RAM** | 4 ГБ | Для небольших файлов и последовательной обработки |
| **Диск** | **260 ГБ** | Пик 253 ГБ, постоянно ~150 ГБ |
| **Сеть** | 10 Мбит/с | Загрузка ~60 часов |
| **ОС** | Linux/Windows Server | Ubuntu 20.04+ / Windows Server 2019+ |
...
| **Диск (с удалением)** | **300 ГБ SSD** | Экономия ~50 ГБ, постоянно ~150 ГБ |
...
Типичная структура данных `/Клиенты`:
```
70 ГБ  - видео файлы (остаются, не удаляются)
15 ГБ  - большие файлы >1 ГБ (не конвертируются, остаются)
20 ГБ  - изображения, бинарники (не конвертируются, остаются)
25 ГБ  - Markdown файлы (вместо 110 ГБ документов Word/Excel/PDF/PPT)
1 ГБ   - база данных SQLite и логи
─────────
≈ 131 ГБ данные
+ 20 ГБ - резерв на рост
─────────
≈ 150 ГБ минимум постоянно
```

**Рекомендация:** При использовании `DELETE_ORIGINALS_AFTER_CONVERSION = True`:
- **Минимальный диск:** 260 ГБ (для пикового потребления)
- **Рекомендуемый:** 300 ГБ (с запасом для комфортной работы)
- **После первой синхронизации** будет занято только ~130-150 ГБ постоянно
